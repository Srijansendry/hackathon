                ...
                <div className="flex gap-1 bg-surface-elevated rounded-lg p-1">
                  {[['week', 'Week'], ['month', 'Month'], ['year', 'Year']].map(([val, label]) => (
                    <button key={val} onClick={() => setFilter(val)}
                      className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all cursor-pointer ${filter === val ? 'dark:bg-slate-700 text-text-heading shadow-sm bg-[color:var(--color-emerald-300)] opacity-[0.79]' : 'text-text-muted hover:text-text-secondary'}`}>
                      {label}
                    </button>
                  ))}
                </div>
