                      ...
                      <div className="flex gap-1 bg-surface-elevated rounded-lg p-1">
                        {[['week', 'Week'], ['month', 'Month'], ['year', 'Year']].map(([val, label]) => (
                          <button key={val} onClick={() => setReadingsFilter(val)}
                            className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all cursor-pointer ${readingsFilter === val ? 'dark:bg-slate-700 text-text-heading shadow-sm opacity-[0.79] bg-[color:var(--color-emerald-300)]' : 'hover:text-text-secondary bg-[color:var(--color-emerald-300)] text-[color:var(--color-slate-400)]'}`}>
                            {label}
                          </button>
                        ))}
                      </div>
