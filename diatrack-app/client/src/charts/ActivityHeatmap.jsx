    ...
    <div className="bg-surface-card rounded-2xl border border-surface-border p-6 shadow-soft hover:shadow-card transition-shadow duration-300">
      ...
      <div className="flex items-start gap-3 overflow-x-auto pb-2">
        ...
        <div className="flex-1 flex flex-col space-y-1">
          {/* Month labels */}
          <div className="flex justify-between text-[9px] font-bold text-text-muted select-none mb-1 px-1">
            <span>{days[0]?.month}</span>
            <span>{days[Math.round(days.length * 0.3)]?.month}</span>
            <span>{days[Math.round(days.length * 0.6)]?.month}</span>
            <span>{days[days.length - 1]?.month}</span>
          </div>
